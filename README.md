...
- **Отказоустойчивость.** Репликация PostgreSQL (streaming replication) + Redis Sentinel (3 узла) для автоматического переключения при сбое узла. Критично для сценариев, когда CDC-пайплайн не должен терять события. Ветка `feature/fault-tolerance`. Подробнее в [`FAULT_TOLERANCE.md`](FAULT_TOLERANCE.md)
...
| **Отказоустойчивость** | Репликация PostgreSQL (streaming replication) + Redis Sentinel |
...
| [`FAULT_TOLERANCE.md`](FAULT_TOLERANCE.md) | Отказоустойчивость: PostgreSQL репликация и Redis Sentinel |
